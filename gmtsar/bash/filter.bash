...
if [ $# -ne 4 ] && [ $# -ne 6 ]; then
  ...
  echo " "
  echo "Example: filter.bash IMG-HH-ALPSRP055750660-H1.0__A.PRM IMG-HH-ALPSRP049040660-H1.0__A.PRM 300  2"
  echo ""
  exit 1
fi
echo "filter.bash"
...
#
# the range spacing in units of image range pixel size
#
if [[ -n $rng_samp_rate ]]; then
  if [ $rng_samp_rate -gt 110000000 ]; then
    dec_rng=4
    filter1=$sharedir/filters/gauss15x5
  elif [ $rng_samp_rate -lt 110000000 ] && [ $rng_samp_rate -gt 20000000 ]; then
    dec_rng=2
    filter1=$sharedir/filters/gauss15x5
    #
...
#
# az_lks and dec_rng to 1 for odd decimation
#
if [ $# -eq 6 ]; then
  jud=$(echo $6 | awk '{if($1%2 == 0) print 1;else print 0}')
  if [ $jud -eq 0 ]; then
    az_lks=1
  fi
  jud=$(echo $5 | awk '{if($1%2 == 0) print 1;else print 0}')
  if [ $jud -eq 0 ]; then
    dec_rng=1
  fi
fi
...
#
#  make the custom filter2 and the decimation
#
...
#
##ln -s ../../topo/dem.grd .
##tide_correction.bash $1 $2 dem.grd
##mv tide.grd tmp.grd
