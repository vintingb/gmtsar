...
if [ $# -lt 2 ]; then
  ...
  echo " "
  echo "Usage: snaphu.bash correlation_threshold maximum_discontinuity [<rng0>/<rngf>/<azi0>/<azif>]"
  echo ""
  echo "       correlation is refito zero when < threshold"
  echo "       maximum_discontinuity enables phase jumps for earthquake ruptures, etc."
  echo "       set fimaximum_discontinuity=0 for continuous phase such as interseismic "
  echo ""
  echo "Example: snaphu.bash .12 40 1000/3000/24000/27000"
  echo ""
  echo "Reference:"
...
# fitmp = $(gmt grdinfo -C -L2 unwrap.grd)
# filimitU=$(echo $tmp | awk '{printf("%5.1f", $12+$13*2)}')
# filimitL=$(echo $tmp | awk '{printf("%5.1f", $12-$13*2)}')
# fistd=$(echo $tmp | awk '{printf("%5.1f", $13)}')
