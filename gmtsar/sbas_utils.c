...
void dgelsy_(const int64_t *m, const int64_t *n, const int64_t *nrhs, double *G, const int64_t *lda, double *b,
             const int64_t *ldb, int64_t *jpvt, const double *rcond, const int64_t *rank, double *work,
             const int64_t *lwork,
             const int64_t *info);
...
int
allocate_memory_ts(int64_t **jpvt, double **work, double **d, double **ds, float **bperp, char ***gfile, char ***cfile,
                   int64_t **L, double **time, int64_t **H, double **G, double **A, double **Gs, int64_t **flag,
                   float **dem,
                   float **res, float **vel, float **phi, float **var, float **disp, int64_t n, int64_t m,
                   int64_t lwork,
...
int init_array_ts(double *G, double *Gs, float *res, float *dem, float *disp, int64_t n, int64_t m, int64_t xdim,
                  int64_t ydim,
                  int64_t N, int64_t S) {
...
int read_table_data_ts(void *API, FILE *infile, FILE *datefile, char **gfile, char **cfile, int64_t *H, float *bperp,
    ...
    for (i = 0; i < N; i++) {
        if ((CC = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, cfile[i],
                                NULL)) ==
            NULL)
        ...
        if ((GG = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, gfile[i],
                                NULL)) ==
            NULL)
        ...
        if ((CC = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, cfile[i],
                                CC)) == NULL)
            die("Can't read ", cfile[i]);
        if ((GG = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, gfile[i],
                                GG)) == NULL)
        ...
        for (k = 0; k < ydim; k++) {
            for (j = 0; j < xdim; j++) {
                ...
                if (corin[j + k * xdim] >= 1e-2 && corin[j + k * xdim] <= 0.99) {
                    ...
                    var[indx] =
                            sqrt((1.0 - corin[j + k * xdim] * corin[j + k * xdim]) /
                                 (corin[j + k * xdim] * corin[j + k * xdim]));
...
int init_G_ts(double *G, double *Gs, int64_t N, int64_t S, int64_t m, int64_t n, int64_t *L, int64_t *H, double *time,
              float sf,
...
int64_t
lsqlin_sov_ts(int64_t xdim, int64_t ydim, float *disp, float *vel, int64_t *flag, double *d, double *ds, double *time,
            ...
            if (flag[j + xdim * k] != 1) {
                ...
                */
                ...
                if (count > 2) {
                    ...
                    vel[j + xdim * k] =
                            -79.58 * wl * (count * sumxy - sumx * sumy) / (count * sumxx - sumx * sumx) * 365.0;
                    if (flag_rms == 1) {
                        aa = sumy / count -
                             (count * sumxy - sumx * sumy) / (count * sumxx - sumx * sumx) * sumx / count;
                        for (i = 2; i < S - 2; i++) {
                            if (atm_rms[i] != 0.0) {
                        ...
                        res[j + xdim * k] =
                                sqrt(count * sumyy / ((count - 2) * (count * sumxx - sumx * sumx))) *
                                (79.58 * wl * 365);
                    }
                } else {
                    for (i = 0; i < S; i++) {
                    ...
                    if (flag_rms == 1) {
                        ...
                        res[j + xdim * k] =
                                sqrt(S * sumyy / ((S - 2) * (S * sumxx - sumx * sumx))) * (79.58 * wl * 365);
                    }
                }
            } else {
                for (i = 0; i < S; i++)
...
int write_output_ts(void *API, struct GMT_GRID *Out, int64_t agc, char **agv, int64_t xdim, int64_t ydim, int64_t S,
                    int64_t flag_rms, int64_t flag_dem, float *disp, float *vel, float *res, float *dem, float *screen,
                    double wl,
...
int free_memory_ts(int64_t N, float *phi, float *var, char **gfile, char **cfile, float *disp, double *G, double *A,
                   double *Gs,
...
int connect(int64_t *L, int64_t *H, double *time, int64_t *hit, int64_t *mark, int64_t N, int64_t S, int64_t n,
            int64_t mode) {
