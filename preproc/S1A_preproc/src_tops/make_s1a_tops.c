...
double shift_write_slc(void *, struct PRM *, struct tree *, burst_bounds *, int, TIFF *, FILE *, FILE *, FILE *, FILE *,
                       char *, char *);
...
int main(int argc, char **argv) {
    ...
    if (imode == 1 || imode == 2 || imode == 3) {
        spec_sep =
                shift_write_slc(API, &prm, xml_tree, bb, imode, TIFF_FILE, OUTPUT_SLCL, OUTPUT_SLCC, OUTPUT_SLCH,
                                OUTPUT_RMP, rshifts, ashifts);
    }
    /* shift applied */
...
double shift_write_slc(void *API, struct PRM *prm, struct tree *xml_tree, struct burst_bounds *bb, int imode, TIFF *tif,
    ...
    if (dr_table[0] != '\0' && da_table[0] != '\0') {
        ...
        if ((R = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, dr_table,
                               NULL)) ==
            NULL)
            die("cannot open range shift tables", dr_table);
        if ((A = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, da_table,
                               NULL)) ==
            NULL)
...
int compute_eap(fcomplex *cramp, tree *xml_tree, int nb) {
    ...
    Worb = 2.0 * M_PI / Torb;
    t1 = t_brst - anx_time;
    height = H[0] + H[1] * sin(Worb * t1 + phi[1]) + H[2] * sin(Worb * 2.0 * t1 + phi[2]) +
             H[3] * sin(Worb * 3.0 * t1 + phi[3]) +
    ...
    for (ii = 0; ii < n_samples; ii++) {
        ...
        if (jj < 1 || jj > srtcount) {
            ...
            real = (Geap[(jj - 1) * 2] * (theta[ii] - theta_eap[jj - 1]) + Geap[jj * 2] * (theta_eap[jj] - theta[ii])) /
                   dtheta;
            imag = (Geap[(jj - 1) * 2 + 1] * (theta[ii] - theta_eap[jj - 1]) +
                    Geap[jj * 2 + 1] * (theta_eap[jj] - theta[ii])) /
                   dtheta;
...
int
shift_burst(fcomplex *cbrst, fcomplex *cbrst2, int al_start, int lpb, int width, struct GMT_GRID *R, struct GMT_GRID *A,
            int bshift) {
    ...
    for (ii = 0; ii < lpb; ii++) {
        for (jj = 0; jj < width; jj++) {
            k = ii * width + jj;
            if (floor((al_start + ii) / incy + 0.5) < 0 ||
                (int) floor((al_start + ii) / incy + 0.5) >= R->header->n_rows) {
                cbrst[k].r = 0;
                cbrst[k].i = 0;
            } else {
